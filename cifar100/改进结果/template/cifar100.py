...
class BasicBlock(nn.Module):
    def __init__(self, in_planes, planes, kernel_size=3, stride=1, act_func='h_swish', drop_connect_rate=0.0):
        super(BasicBlock, self).__init__()

        self.sep_conv = nn.Conv2d(in_planes, in_planes, kernel_size=kernel_size, stride=stride, padding=int((kernel_size-1)/2), bias=False,groups=in_planes)
        self.bn2 = nn.BatchNorm2d(in_planes)


        self.point_conv = nn.Conv2d(in_planes, planes, kernel_size=1, stride=stride, padding=0, bias=False,groups=1)
        ...
        if act_func == 'relu':
            ...
            self.se = ECALayer(in_planes, in_planes)
        self.drop_connect_rate = drop_connect_rate

    def forward(self, x):
        out = self.act_func(self.bn2(self.sep_conv(x)))
        out = self.se(out)
...
class TrainModel(object):
    def __init__(self, is_test, batch_size, weight_decay):
        if is_test:
            full_trainloader = data_loader.get_train_loader('/data3/houjiao/EPCNAS/EPCNAS1/datasets/CIFAR100_data/', batch_size=batch_size, augment=True,shuffle=True, random_seed=2312391, show_sample=False,num_workers=4, pin_memory=True)
            testloader = data_loader.get_test_loader('/data3/houjiao/EPCNAS/EPCNAS1/datasets/CIFAR100_data/', batch_size=batch_size, shuffle=False,num_workers=4, pin_memory=True)
            self.full_trainloader = full_trainloader
            self.testloader = testloader
        else:
            trainloader, validate_loader = data_loader.get_train_valid_loader('/data3/houjiao/EPCNAS/EPCNAS1/datasets/CIFAR100_data/', batch_size=256,augment=True, subset_size=1,valid_size=0.1, shuffle=True,random_seed=2312390, show_sample=False,num_workers=4, pin_memory=True)
